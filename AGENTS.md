...
4. **Voice picker buttons are visual-only in `VoicePicker.tsx`** — intentional. Buttons select a Gemini voice ID but don't play audio previews yet. Wire real TTS previews when backend is ready.

## TODO

- **Voice preview audio for Puck + Algenib** — Generate 2 short TTS samples via Gemini REST API (`gemini-2.5-flash-preview-tts`), save as `.wav` in `ui/public/voices/`, and wire `VoicePicker.tsx` to play them on click for the first 2 options (Upbeat, Gravelly). The other 3 stay visual-only for now.
